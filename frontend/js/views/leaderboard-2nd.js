...
// --- 상태 변수 ---
let currentStage = "2nd-qualifying";
...
const tournamentSchedule = [
  { name: "[예선전]", start: "2025-10-24", end: "2025-11-09" },
  { name: "[64강]", start: "2025-11-10", end: "2025-11-16" },
  { name: "[32강]", start: "2025-11-17", end: "2025-11-23" },
  { name: "[16강]", start: "2025-11-24", end: "2025-11-27" },
  { name: "[8강]", start: "2025-11-28", end: "2025-12-01" },
  { name: "[4강]", start: "2025-12-02", end: "2025-12-04" },
  { name: "[결승/3위전]", start: "2025-12-05", end: "2025-12-07" },
];

const courseInfoByDate = {
  예선: ["블루원 상주 골프리조트", "골프존카운티 더골프", "하모니 골프 파크"],
  "64강": ["블루원 용인 CC 동/서", "해운대 CC GOLDEN/ROYAL"],
  "32강": ["썬밸리 CC (일죽)", "구미 CC 청룡/거북"],
  "16강": ["안동레이크 GC", "한맥 C.C & 노블리아"],
  "8강": ["메이플비치 골프&리조트", "스프링데일 CC"],
  준결승: ["포레스트힐 CC", "노벨 CC 충무/가야"],
  결승: ["대관령힐스 GGC", "샴발라 CC"],
};
 ...
 * @param {string} stage - 렌더링할 스테이지 (e.g., '2nd-qualifying', '2nd-64')
 * @param {HTMLElement} app - 앱의 루트 요소
 */
export function renderLeaderboard2ndPage(stage, app) {
  currentStage = stage;
...
function renderHeader(header, stage) {
  ...
  header.innerHTML = `
        <div class="title-container">
            ...
            <div class="subtitle">
                <span class="subtitle-text">골목대장 토너먼트 2nd - ${
...
function renderContent(container) {
  ...
  if (activeTab === "total") {
    if (currentStage === "2nd-qualifying" && currentViewMode === "bracket") {
...
function renderLeaderboardView(container, data) {
  const isQualifying = currentStage === "2nd-qualifying";
  const headersConfig = {
    total: [
      { key: "순위" },
      { key: "닉네임" },
...
function renderCutoffView(container, data) {
  ...
  // 2nd 토너먼트는 64명 컷오프
  const cutoffData = data.slice(0, 64);
  let columns = [[], [], [], [], [], [], [], []]; // 8개 컬럼으로 변경
...
async function renderFinalStageWithThirdPlace(container) {
  try {
    ...
    const [finalData, thirdPlaceData] = await Promise.all([
      fetchStageData("2nd-final"),
      fetchStageData("2nd-third-place"),
    ]);
...
async function renderFullBracket() {
  ...
  const rounds = [
    { name: "64강", class: "round-of-64", players: 64 },
  ...
  try {
    const [data64, data32, data16, data8, data4, dataThirdPlace, dataFinal] =
      await Promise.all([
        fetchStageData("2nd-64"),
        fetchStageData("2nd-32"),
        fetchStageData("2nd-16"),
        fetchStageData("2nd-8"),
        fetchStageData("2nd-4"),
        fetchStageData("2nd-third-place"),
        fetchStageData("2nd-final"),
      ]);
